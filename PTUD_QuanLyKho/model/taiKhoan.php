...
class TaiKhoan{
    ...
    function doianh($maTaiKhoan, $anh){
        $stmt = $this->conn->prepare("UPDATE taikhoan set AnhDaiDien = :pass WHERE MaTaiKhoan = :maTaiKhoan");
        $stmt->bindParam(':maTaiKhoan', $maTaiKhoan, PDO::PARAM_INT);
        $stmt->bindParam(':pass', $anh, PDO::PARAM_STR);
        $result = $stmt->execute();
        return $result ?: false;
    }
}
